...
- Search, Automation, Share, and Import/Export form the right-aligned utility
  cluster of the Layout bar. Search expands into a compact fixed-width control
  on that bar; it may reduce the visible tab viewport but must not change the
  bar height or push the work area.
...
- Pressing Enter in Database search enables `Only matches`, closes suggestions,
  and moves focus/viewport to the first ranked result.
- `Ctrl/Cmd+Z` restores the latest reversible Database revision only when focus
  is outside text inputs and content editors; native text undo always wins.
...
- Dragging never creates a cursor-following native or application ghost. The
  source remains marked in place. Dense work areas may preview the final slot
  in place; nested context trees must keep their structure stable until drop
  and only highlight the current destination. `Escape` restores the pre-drag
  state.

## Grouped data and pagination

- Group headers are sticky while their entities scroll. The count beside a
  header is the server total, never merely the number of loaded rows.
- Each expanded group owns its own bounded pagination window and Load more
  action. Collapsed groups do not eagerly fetch Entity pages.
- Group order is persisted per Layout. Users can reorder group headers directly
  in the work area or from the Group settings page. A compact Collapse all
  action lives in View Controls; opening groups remains an intentional
  per-group action so large data sets are not fetched accidentally.
- A sticky group header spans the full visible grid and paints above every
  frozen column at all horizontal scroll positions.
- Direct Entity ordering changes Sort to `Custom`; Reset restores the active
  View Preset and Save persists the new view state.

## History and import review

- Change History is an overlay drawer and must not resize the Database canvas.
  Each entry states what changed and when; a restore requires an explicit
  second confirmation and a restored entry is visibly non-actionable.
- Import mapping shows three explicit outcomes per source column: create a new
  Field, map to an existing Field, or `Don't Import`. When an existing Field is
  selected, show its type as locked rather than an editable second dropdown.
- New choice Fields preview the options that will be created. The required Name
  mapping is visually distinct; UID remains generated by the Database.
- Invalid conversion review must use the complete invalid-ID set, independent
  of normal table pagination. Closing the review restores the pending
  conversion dialog and its preview rather than discarding user decisions.

## Scrollbars

- Vertical and horizontal work-area scrollbars use the compact application
  treatment (6 px track, rounded thumb, stronger hover contrast) consistently
...
- Changing an editable Field type always opens a portalled preview before
  ...
  loss explicitly. Offer Filter invalid items and Change Anyway; the latter
  preserves auditability with a unique `WRONG FORMAT n` Entity Name. The
  preview is read-only and must not mutate the Field or
...
- The app shell has no global topbar. App Rail, Context Sidebar and Work Area
  all begin at the viewport top so the Work Area owns the full available
  height. When a Database is opened through a Space placement, show its
  location breadcrumb but not a second database title/description header.
- User identity occupies the bottom slot of the App Rail. Its menu owns
  workspace switching, notifications, invitations, account settings and
  logout. When the Context Sidebar is closed, use one compact floating reopen
  control instead of reserving a permanent toolbar row.
...
- Checking an individual row enters row-selection mode: clear the active cell
  range and add or remove that row from the existing selection without a
  modifier key. Shift-check adds or removes a contiguous range. The header
  checkbox also clears any active cell range.
- Search, Filter, Sort, Group, and other view-state changes never clear an
  explicit row selection. Search is applied before server pagination so result
  counts and `Only matches` are independent of the loaded page window.
- Row content is vertically centred within the 30 px row rhythm. Selection
  ...
  or ring.
- Database chrome density is fixed: Database Context, Layout bar and View
  controls are 28 px each, with no global topbar above them. Use the matching
  compact font/icon scale rather than shrinking only the container.
...
- Board initially exposes at most ten columns and ends the horizontal sequence
  with a compact Load more groups target. Each column owns its Entity paging
  and exact total; never reuse the Database-wide loaded count as a group count.
  The canonical Name is the card title. Choice-backed group columns inherit the
  original option color with a restrained surface tint and stronger top edge.
- Conditional color is Layout state. Custom rules may color rows/cards and
  group headers; tag-derived mode reuses the original Select, Status,
  Multi-select, or Priority option color rather than generating a second
  palette.
